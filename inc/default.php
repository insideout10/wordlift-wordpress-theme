...
function theme_localization () {
	load_theme_textdomain( 'wordlift', get_template_directory() . '/languages' );
}
...
function social_buttons() {
    ...
    if( get_field( 'footer_social_buttons', 'option' ) ){
        ...
        $social_buttons = '<ul class="social-network">';

            while( has_sub_field( 'footer_social_buttons', 'option' ) ){
                $name = get_sub_field( 'name' );
                $link = get_sub_field( 'link' );

                $social_buttons .= '<li><a href="'.$link.'" target="_blank"><span class="icon icon-'.strtolower( $name ).'"></span></a></li>';
            }
...
function register_shortcodes(){
   add_shortcode('social-buttons', 'social_buttons');
}
...
add_filter( 'body_class', 'wordlift_class_names' );
function wordlift_class_names( $classes )
{
	if( is_page_template('pages/template-home.php') ){ $classes[] = 'large-page'; }
...
function wp_admin_bar_valid_search_menu( $wp_admin_bar ) {
	...
	$form .= '<input type="submit" class="adminbar-button" value="' . __( 'Search', 'wordlift' ) . '"/>';
	$form .= '</div></form>';
...
function theme_get_the_password_form() {
	...
	<p>' . __( 'This content is password protected. To view it please enter your password below:', 'wordlift' ) . '</p>
	<p><label for="' . $label . '">' . __( 'Password:', 'wordlift' ) . '</label> <input name="post_password" id="' . $label . '" type="password" size="20" /> <input type="submit" name="Submit" value="' . esc_attr__( 'Submit', 'wordlift' ) . '" /></p></form>
	';
	return $output;
}
