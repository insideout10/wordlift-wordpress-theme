...
function initScrollToSection() {
	...
	if (hash.length) {
		var section = jQuery(hash.replace('-section', ''));

		if (section && section.length) {
			var topPosition = section.offset().top - headerHeight;

			setTimeout(function() {
				SmoothScroll.scrollTo(topPosition, 600);
			}, delay);
		}
	}
}
...
function initLightbox() {
	jQuery('a.lightbox').each(function() {
		...
		var modal = jQuery(link.attr('href'));
...
function initVideoBg() {
	jQuery('.video-box').videoBG();
}

// initialize fixed blocks on scroll
function initFixedScrollBlock() {
	jQuery('.two-columns').fixedScrollBlock({
		slideBlock: '.sidebar',
		positionType: 'fixed',
...
function initSameHeight() {
	jQuery('.plan-two-columns').sameHeight({
		elements: '.col-hold',
		flexible: true,
		multiLine: true,
		biggestHeight: true
	...
	VideoBG.prototype = {
		init: function() {
			if (this.options.holder) {
				this.findElements();
				this.buildPoster();
				if (!isTouchDevice && this.video.length && this.video.length) {
					this.attachEvents();
		...
		findElements: function() {
			...
			this.ratio = this.video.width() / this.video.height();
			if (this.options.autoPlay) {
				this.video.attr('autoplay', '');
			}
		},
		initSource: function() {
			this.video.children().each(function() {
		...
		attachEvents: function() {
			...
			this.makeCallback('onReady', true);
		},
...
*/
!function(t){t.fn.fitVids=function(e){var i={customSelector:null};if(!document.getElementById("fit-vids-style")){var r=document.createElement("div"),a=document.getElementsByTagName("base")[0]||document.getElementsByTagName("script")[0],o="&shy;<style>.fluid-width-video-wrapper{width:100%;position:relative;padding:0;}.fluid-width-video-wrapper iframe,.fluid-width-video-wrapper object,.fluid-width-video-wrapper embed {position:absolute;top:0;left:0;width:100%;height:100%;}</style>";r.className="fit-vids-style",r.id="fit-vids-style",r.style.display="none",r.innerHTML=o,a.parentNode.insertBefore(r,a)}return e&&t.extend(i,e),this.each(function(){var e=["iframe[src*='content.jwplatform.com']","iframe[src*='player.vimeo.com']","iframe[src*='youtube.com']","iframe[src*='youtube-nocookie.com']","iframe[src*='kickstarter.com'][src*='video.html']","object","embed"];i.customSelector&&e.push(i.customSelector);var r=t(this).find(e.join(","));r=r.not("object object"),r.each(function(){var e=t(this);if(!("embed"===this.tagName.toLowerCase()&&e.parent("object").length||e.parent(".fluid-width-video-wrapper").length)){var i="object"===this.tagName.toLowerCase()||e.attr("height")&&!isNaN(parseInt(e.attr("height"),10))?parseInt(e.attr("height"),10):e.height(),r=isNaN(parseInt(e.attr("width"),10))?e.width():parseInt(e.attr("width"),10),a=i/r;if(!e.attr("id")){var o="fitvid"+Math.floor(999999*Math.random());e.attr("id",o)}e.wrap('<div class="fluid-width-video-wrapper"></div>').parent(".fluid-width-video-wrapper").css("padding-top",100*a+"%"),e.removeAttr("height").removeAttr("width")}})})}}(window.jQuery||window.Zepto);
